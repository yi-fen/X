...
@interface RRZJURLProtocol : NSURLProtocol <NSURLSessionDataDelegate>
@end
...
// 额外Hook AFNetworking确保万无一失
@interface AFHTTPSessionManager : NSObject
@end

@implementation NSObject (AFNetworkingHook)
...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (afClass) {
            ...
            Method originalMethod = class_getInstanceMethod(afClass, selector);
            IMP originalIMP = method_getImplementation(originalMethod);
            
            IMP hookedIMP = imp_implementationWithBlock(^id(AFHTTPSessionManager *self, SEL _cmd, NSURLRequest *request, id uploadProgress, id downloadProgress, void (^completionHandler)(NSURLResponse *, id, NSError *)) {
                // 这里不需要额外处理，因为NSURLProtocol已经拦截了所有请求
                return ((id (*)(id, SEL, NSURLRequest *, id, id, void (^)(NSURLResponse *, id, NSError *)))originalIMP)(self, _cmd, request, uploadProgress, downloadProgress, completionHandler);
            });
